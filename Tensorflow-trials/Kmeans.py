...
srcdata1, label = loadDataSet()
center,result = KMeansCluster(srcdata1,k)
print(center)
############利用seaborn画图###############

res={"x":[],"y":[],"class":[]}
for i in range(len(result)):
    res["x"].append(srcdata1[i][0])
    res["y"].append(srcdata1[i][1])
    res["class"].append(result[i])
pd_res=pd.DataFrame(res)
sns.lmplot("x","y",data=pd_res,fit_reg=False,size=5,hue="class")
plt.show()
